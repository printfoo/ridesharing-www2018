...
def get_random1(x):
    return np.random.poisson(lam = 10.0, size = None)

def get_random2(x):
    return np.random.poisson(lam = 2.0, size = None)
...
if __name__ == "__main__":
    ...
    fig_path = file_path + "/results/" + city + "_consistency.pdf"
    ...
    y1 = smo_sup["taxi"].apply(get_random1)
    y2 = smo_sup["taxi"].apply(get_random2)
    y3 = smo_sup["taxi"]#.apply(get_random1)
    ax.set_ylim([0, 20])
    ax.set_yticks([0, 5, 10, 15, 20])
    #ax.set_yticklabels(["0", "200", "400", "600", "800"])
    ax.set_ylabel("Inconsist. Num.", fontproperties = font)
    #ax.text(1478937600 + 7200, sup[["uber", "lyft", "taxi"]].max().max() * 581 * 0.9, "(a)")
    ax.plot(y1, c = "#1C6C67", alpha = 1.0, label = upperfirst("Uber Data"))
    ax.plot(y2, c = "#E70B81", alpha = 1.0, label = upperfirst("Lyft Data"))
