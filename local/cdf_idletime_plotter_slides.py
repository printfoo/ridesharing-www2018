...
if __name__ == "__main__":
    ...
    fig_path1 = file_path + "/results/idletime_vio_slides1.png"
    fig_path2 = file_path + "/results/idletime_vio_slides2.png"
    ...
    # Plot.
    fig, ax = plt.subplots(nrows = 1, ncols = len(slot_list), figsize=(8, 1.2))
    ...
    for i in range(len(slot_list)):
        handle = ax[i].violinplot(vio[slot_list[i]], showmeans = False, showmedians = True)
        ...
        ax[i].set_xticklabels(["Uber", "Lyft", "Taxi"])
        ax[i].set_xlabel(str(slot_list[i]) + ":00 - " + str((slot_list[i] + 4) % 24) + ":00",
                         fontproperties = font)
        ax[i].set_ylim([-0.2, np.log10(800000)])
        if i == 0:
            ax[i].text(0.75, 0.82 * np.log10(800000), "(c)", fontproperties = font)
            ax[i].set_yticks([0, np.log10(60), np.log10(3600), np.log10(86400 / 2)])
            ax[i].set_yticklabels(["1s", "1m", "1h", "12h"])
            ax[i].set_ylabel("Idle Time", fontproperties = font)
        else:
            ax[i].set_yticks([])
