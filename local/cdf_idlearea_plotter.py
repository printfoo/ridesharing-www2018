...
if __name__ == "__main__":
    ...
    ax.set_xlabel("Number of Visited Block Groups", fontproperties = font)
    #ax.set_xscale("log")
    #ax.plot(np.log10(uber_df["idlearea"]), uber_df["cdf"], color = c_dict["uber"], label = "Uber")
    ax.plot(lyft_df["idlearea"], lyft_df["cdf"], color = c_dict["lyft"], label = "Lyft")
    ax.plot(taxi_df["idlearea"], taxi_df["cdf"], color = c_dict["taxi"], label = "Taxi")
